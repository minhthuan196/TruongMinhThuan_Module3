...
select *
from (
	select acs.id, ifnull(sum(dc.quantity),0) as total_accompanied_service
	from detail_contract dc
	join accompanied_service acs on acs.id = dc.accompanied_service_id
	group by acs.id
) as total_accompanied_service_table
having total_accompanied_service = max(total_accompanied_service)
;
